<template>
  <div class="container">
    <Breadcrumb :items="['账号列表', '交易账号']" />
    <a-card class="general-card" title="交易账号">
      <a-row>
        <a-col :flex="1">
          <a-form
      ...
      >
        <template #columns>
          ...
          <a-table-column title="仓位因子" data-index="factor">
            <template #cell="{ record }">
              {{ 'x' + record.factor }}
            </template>
          </a-table-column>
          <a-table-column title="操作" data-index="operations">
            <template #cell="{ record }">
              <a-space>
                <a-button type="primary" @click="toPreviewData(record.id)">
                  数据统计
                </a-button>
                <a-button @click="toEdit(record.id)"> 修改 </a-button>
              </a-space>
            </template>
          </a-table-column>
        </template>
...
<script lang="ts" setup>
  ...
  function toPreviewData(id: number) {
    router.push({
  ...
  const fetchData = async (params: PageParam = { page: 0, size: 10 }) => {
    setLoading(true);
    try {
      const { data } = await queryTradeAccountList(params);
